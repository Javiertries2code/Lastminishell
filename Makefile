...
# Archivos fuente
LEXER_FILES = copy_env.c kloc.c lexer.c main_support.c split_quotes.c expand_var.c expand_var_ext.c copy_env_ext.c 
...
SRC := $(LEXER) $(HANDLERS) $(PARSERS) $(EXITING) $(EXECUTING) $(MANAGEMENT) $(BUILTINS) main.c
OBJECTS := $(SRC:.c=.o)
...
debug_leaks:
	$(MAKE) fclean
	$(MAKE) DEBUG=1
	valgrind --leak-check=full --show-leak-kinds=all --suppressions=readline.supp --track-origins=yes ./$(NAME) $(ARGS)
