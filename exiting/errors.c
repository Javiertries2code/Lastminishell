...
/**
 * @brief Validates initial syntax of input line
 * @param data Main data structure containing program state
...
int	check_initial_errors(t_data *data, char *line)
{
	...
	if (quotes_balanced(line) == false)
		return (return_error(WRONG_SYNTAX, "unmatched quotes", data));
	...
	if (trimmed[0] == '|' || trimmed[ft_strlen(trimmed) - 1] == '|')
	{
		...
		return (return_error(WRONG_SYNTAX, "invalid pipe position", data));
	}
	free_str_safe(&trimmed);
	return (0);
}
...
/**
 * @brief Validates command tokens for syntax errors
 * @param data Main data structure containing program state
 * @param token Token list to validate
 *
...
/**
 * @brief Validates all command token arrays
 * @param data Main data structure containing program state
 *
